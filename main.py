...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
    "https://respidata-frontend.vercel.app",  # Frontend en ligne
...
def get_uv_data(lat, lon):
    # Vérifie l'heure locale avant d'appeler l'API
    try:
        tf = TimezoneFinder()
        timezone_str = tf.timezone_at(lat=lat, lng=lon)
        if timezone_str:
            tz = pytz.timezone(timezone_str)
            heure_locale = datetime.now(tz).hour
            if heure_locale < 6 or heure_locale > 20:
                print(f"🌙 UV ignoré (nuit détectée à {heure_locale}h) → uvi = 0.0")
                return 0.0
    except Exception as e:
        print(f"❌ Erreur lors du calcul de l'heure locale pour l'UV : {e}")
    ...
    try:
        response = requests.get(url)
        data = response.json()
        uvi = data.get("value", 0.0)
        print(f"☀️ UV API 2.5 → uvi = {uvi}")
        return uvi
    except Exception as e:
        print(f"❌ Erreur lors de l'appel UV API 2.5 : {e}")
        return 0.0
...
def generate_full_summary(data: FullSummaryRequest):
    lat = data.lat
    lon = data.lon
